...
import { ADDate, addDays, getWeekdayFromAD, monthLengths, parseDate, yearStart } from "./helpers";

export function bsToAd(bsInput: string | { year: number; month: number; day: number }): ADDate | { error: string } {
    try {
        ...
        const daysPassed = lengths.slice(0, month - 1).reduce((sum: any, n: any) => sum + n, 0) + (day - 1);
